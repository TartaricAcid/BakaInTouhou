...
// 来自 MCP 中同名方法，很遗憾原方法是硬编码原木和叶子
// 部分方法连继承都无法做到，我只能完全 copy 一份来这么做了
public class WorldGenBigTree extends WorldGenAbstractTree {
    ...
    public WorldGenBigTree(boolean notify, IBlockState leaf, IBlockState wood) {
        super(notify);
        this.leaf = leaf;
        this.wood = wood;
    }
    ...
    void generateLeafNode(BlockPos pos) {
        for (int i = 0; i < this.leafDistanceLimit; ++i) {
            this.crosSection(pos.up(i), this.leafSize(i), leaf.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeafNodeBases() {
        for (WorldGenBigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
            ...
            if (!blockpos.equals(worldgenbigtree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY())) {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates, wood.getBlock());
            }
        }
    }
    ...
    private boolean validTreeLocation() {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, BlockObjectHolder.blockSakuraSaplingPink);
